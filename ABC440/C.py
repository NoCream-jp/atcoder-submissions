...
def main():
    ...
    """
    全探索ならNWかかる
    毎回x個飛ばしで集めるのも考えたけどx ∈ W全部やるのは重い

    書いてたらパターンはありそう．
    当然やけど(i + x)の集合のうち同じものしか使ってない
    x変えても同じ項数しか和をとらない
    増えていくから％取りたい

    w一回につきスライド一周分のNしかかからない!
    oxxxoooo
    ooxxxooo
    oooxxxoo
    oooxxxxo
    このoをスライドさせてminとるだけ？
    iにx足すとき足される塊がx個からなる
    x個,1個,x個,1個...(全部でN個)みたいな配置をスライドさせた全通りになる
    ならそれを集めたいがx=2のときNぐらいオーダーかかるのがわかる
    必ず半分になってる
    それの全通りなら総和の半分でいいのでは？と思ったけど穴あいてる

    穴をスライドすればNでいける？
    4こぶんの穴なら4*4回で(N個触らない，端っこの4つだけ)
    iこぶんの穴ならi*4回で
...
def get_primes(l, r):
    ...
    if limit >= 2:
        ...
        for i in range(2, int(math.isqrt(limit)) + 1):
            if is_prime_small[i]:
                is_prime_small[i*i : limit+1 : i] = [False] * len(is_prime_small[i*i : limit+1 : i])
...
def my_bisect_left(a, x):
    if x < a[0]: return None
    if a[-1] < x: return None
