...
def main():
    ...
    二次元DPに見える
    2*N
    dp[0][i] : i匹目をHで倒したときの残りのH,Mの最大値
    dp[1][i] : Mで倒したときの残りのH,Mの最大値

    ↑違う．一回ぶつかるまで体力でやってみて，
    それ以降の表の更新に魔力を使ってみる．
    """
    idx = 0
    dp = [[0 for _ in range(M + 1)] for _ in range(H + 1)]
    ...
    for i in range(1, M + 1):
        if N <= idx:
            break
        elif i == nowcost + A[idx]:
            dp[0][i] = dp[0][i - 1] + 1
            nowcost += A[idx]
            idx += 1
        else:
            dp[0][i] = dp[0][i - 1]
        # print(f"i={i},A[{idx}]={A[idx]}, nowcost={nowcost}")
