...
def main():
    ...
    nextがNなら最小値更新
    ↑ちがう
    Nを通りすぎて値をしてくるパターンもある．Nを終点としないといけないので，
    2周3周は必要ないけど全部見ないといけない
    だからNでちぎった各部分木(Nでつながる閉路は一つにする)全体で持ってる
    XOR値を並べて，1からNまでのものと比べてみる？
    ・閉路無しの場合，行って戻れば葉だけ反映されることになる
    ・閉路アリの場合，その閉路がもってる値をそのままNに持ってこれる．
    ↑途中に小さな閉路があるとつらい．
    ↑ ちょっと違った．1からNまでの経路上のXORが決まってるときは，
    なるべく小さく相殺できるようにN始点の経路を付け足せる．
    道中関係なく，【NからNに帰ってこれる】経路上のXORしか使えない．
    葉まで行ったらもとに戻るために相殺しないといけないので，
    Nに戻ってこれる閉路のみ考える．
    道中に小さな閉路があっても，経路が増えるだけで問題ない．
    どこかを2周するメリットはない
    (寄与XORが0となるなら行かないのと同じ)ので，あとは1からNまでの
    経路をどう出すか
    全部ならべて値でにぶたんはたぶんできるけどしたくない．
    """
