...
def mdist(i1, j1, i2, j2):
    tate = abs(i1 - i2)
    yoko = abs(j1 - j2)
    return max(tate, yoko)


def main():
    """
    ij分けて考える
    それぞれのminとmaxの中間地点を目指すのが答え

    ななめありなら違う．
    集合地点でにぶたんをiとjの二回する？okngの境が無いからにぶたんは無理

    平均点を目指した方が早いに決まっているので，角の2つのうちナナメ込み距離の大きい方を取る
    対角じゃなくてもよい？
    """
    N = int(input())
    mni, mxi, mnj, mxj = float("inf"), 0, float("inf"), 0
    for _ in range(N):
        R, C = i_map()
        if R < mni:
            mni = R
        ...
        if mxj < C:
            mxj = C
    # print()
    # print(mni, mxi, mnj, mxj)
    gi, gj = (mni + mxi) // 2, (mnj + mxj) // 2
    print(max(mdist(mni, mnj, gi, gj), mdist(mxi, mxj, gi, gj)))
