...
def main():
    ...
    """
    # 貪欲に斜めに動いて到達できそう
    縦横に離れていれば斜め移動を繰り返して二回で行ける
    どんなに離れていても三回以内で行けそう
    縦横合わせるために斜めに移動するので一回
    そこから二回折り返してタテ移動
    なので
    ・同じ場合 = 0
    ・初手で行ける場合 = 1
    ...
    ・それ以外 = 3
        ...
        斜め移動二回でどこでも到達できると思う
        ちょうど斜めで到達できる点っていうのは
        斜め移動の後さらに斜め移動一回だけで行けるところなので
        チェス盤でいうところの原点と同じ色のマス
        原点白なら白がOK、黒ならもう一手いる
        言い換えるとx+yが偶数な(x, y)
        さらにいうと座標はabsで考えて良い
    ...
    (4, 5)に到達できるかどうか
    初手で到達できないから0, 1ではない
    4,4に行ってから1マス動くという2手で行ける
    なぜコードでは3手だと出るのか

    偶数でなくとも斜めに動いた後に縦横に3マス動けるコマなのでその動きで一手で詰められる時がある
    なら、rとcの差を見る
    絶対値引けば小さくなるからそのあと
    ...
    if r2 == c2 == 0:
        ans = 0
    elif (r2 == c2) or (abs(r2) + abs(c2)) <= 3:
        ans = 1
    elif (r2 + c2) % 2 == 0 or abs(r2 - c2) <= 3:
        ans = 2
    else:
        ans = 3
    print(ans)
