...
def main():
    ...
    """
    各カードで(裏-表)を並べたら裏返すのが嬉しい度が出てわかりやすい？
    その嬉しい度リストの中で、K個以内の区間を選んで和を最大化したい

    でも一度選んだ区間内でもう一度裏返すのが最善の場合がある
    +++-+++でK=2なら全部+で取れるとか
    
    セグ木のにおいもDPのにおいもする
    
    K分割してdpで良い気がする
    分割の境界をまたいで二分割以上するケースでダメ

    dpだと遷移がわからなすぎる
    jまで見た時に区間i個裏返したときの最大スコアをdp[i][j]は無理そう

    Kが1なら、区間和を累積和で求められる
    Kが10までなので、K回貪欲に区間和の最大を求めながらその時の端を記録することをすれば、汚いけど解けそう？
    改善の余地なくなったところで止まる、そのまま返るので
    負の数を含む場合の区間和の最大値はわからない
    """
    ...
    # # K回累積和から最大区間を求めることをして、更新して区間を保存すれば復元できる
    # for _ in range(K):
    #     cs = cum_sum(lst[2])
    #     M[0] = 0
