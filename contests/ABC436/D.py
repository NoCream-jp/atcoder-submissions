...
def main():
    ...
    while q:
        ...
        if "a" <= c <= "z":
            if c in teleport:
                for ni, nj in teleport[c]:
                    if visited[ni][nj] == -1:
                        visited[ni][nj] = nowdist + 1
                        q.append((ni, nj))
                del teleport[c]  # 消す ワープ済みに後から到達したら遅い
        # ワープアルファベットから、たてよこ移動もできる
        for di, dj in drct:
            ni, nj = i + di, j + dj
            ...
            if visited[ni][nj] == -1:
                visited[ni][nj] = nowdist + 1
                q.append((ni, nj))
                # print(f"add {ni} {nj} {visited[ni][nj]}")
