...
def main():
    ...
    ・左の0をすべて払った後で長いものが左
        ...
        45
        6
        で、12345になってしまい損（正解は45123）
    ・10桁に変形して辞書で大きいものが左
        0が左にないものばかりなら、短いものを採用してしまい損
    
    ある数字を採用したら、あとは長さ勝負
    採用するやつとその右に続くやつで選び方を変える必要がある？
    Kの値によって変わる。長くなくても先頭の候補になるかもしれないから
    というか0が左にある数字がないケースでは、長い順にK個採用されるのは確定なので

    K == N なら
    辞書順でいい
    そうでないなら
    長いものK個採用がいいが0が左にあると困る
    各文字列について右端からどれだけ離れたところに0以外の数字があるかを見て
    最長のうち辞書順後ろが先頭で確定する？ 
    しない。

    文字列の持つ要素
    辞書内での強さa、長さb、0でない部分の長さc
    
    これに優先度付けしてソートしたい
    ・先頭としての価値のあるものはこのうちcが大きい→aが大きい
    ・それ以降としての価値のあるものはbが大きい→aが大きい
    6
    32
    が反例になってしまうので先頭の決め方は間違い
    ...
    # # 辞書内での強さ、長さ、0でない部分の長さを返してキーにする
    # # 先頭を決めるために0でない部分の長さ優先
    # def f(string):
    #     number = int(string)
    #     length = len(string)
    #     value = len(string)
