...
def main():
    ...
    """
    駒を下げられないから，上に合わせるしかない
    
    左から自分の左を見ていって，くだりなら自分の左を自分まで下げる
    そのあと「自分の高さまで連続している奴ら」を扱っていける
    次のやつが左を見て下げる必要があるなら，それをまとめて処理させればいい

    でものぼりだったとき，そこから上り坂を作るべきか下り坂を作るべきかは左のやつが
    全体の最小値かどうかを見ないといけない
    左のやつが最小値で，次から登りだったならこれまでの下り坂は確定するので捨てる
    実際にRを書き換えてチェックしてみる

    とちゅうで折り返すのが最適な場合がわからないので諦め


    heapqから取り出してlstの自分の位置と左右を埋めていくことにする
    自分の欄に書いてあったら持ってるのを書いてあるのに変更
    Rにおいての左右を見て，大きければlstに自分+1を書き込む
    そうでなければそのままにしておく
    """
    ...
    for t in range(T):
        N = int(input())
        R = i_list()
        tmp = [(R[i], i) for i in range(N)]
        # print(tmp)
        heapq.heapify(tmp)

        lst = [float("inf") for i in range(N)] # 最後これに合わせて答え計算する
        while tmp:
            now, center = heapq.heappop(tmp)
            if lst[center] < now:
                continue
            # print(" ", now, center)
            left, right = -1, -1
            ...
            # 左書き込む
            if left != -1:
                lst[left] = min(lst[left], now + 1)
                heapq.heappush(tmp, (lst[left], left))
            # 右書き込む
            if right != -1:
                lst[right] = min(lst[right], now + 1)
                heapq.heappush(tmp, (lst[right], right))
            print(lst)
        ans = 0
        for i in range(N):
            ans += abs(R[i] - lst[i])
