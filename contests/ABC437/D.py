...
def main():
    """
    二分探索してその左右で計算
    累積和とればa_i*数 - 累積和 と 累積和 - a_i*数でO1
    """
    ...
    for a in A:
        ...
        if idx == 0:
            left = 0
            right = abs((C[-1]) - a * len(C))
        elif idx == len(C):  # 右端だった場合 idx == len(C)
            left = abs(a * len(C) - C[len(C) - 1])
            right = 0
        else:  # 真ん中
            left = abs(a * (idx) - C[idx - 1])
            right = abs((C[-1] - C[idx - 1]) - a * (len(C) - idx))
            # print(f"left=abs(a*{idx} - C[{idx-1}])={left}")
            # print(f"right=abs(a*{len(C)-idx} - {(C[-1] - C[idx-1])}={right}")
        count = left + right
