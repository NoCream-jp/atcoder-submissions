...
def main():
    ...
    """
    当然タイル状に敷き詰めるのが良いので、平方数個で割って余りをいい感じにくっつける。
    余りは角を経由してくっつけることを優先すると、3タイルで4つ増やせる。
    と思ったけど、N以下の平方数がでかすぎる？
    10^18の平方根までしか列挙しないにしても10^9かかる
    Tも大きいのでほぼO(1)

    4で割っていくと嬉しそう

    100を4で割ると
    25, 6, 1, 0 で 3回 (4分割を3回行える)
    3回正方形をなす構造を作れるととれる
    4^3 = 64が出る
    一辺の長さnの正方形部分の嬉しさは頑張る
    あとは角をうまいことやる。
    一つの角について3枚いるので
    100 - 64 = 36 (周りに使うカード)
    36 / 3 = 12 >= 4 (全部の角を覆える)
    36 - 3*4 = 24 (角以外の周りに使うカード)
    この24は角の近くからくっつけるのが嬉しい。もう一つの角に到達するまでの長さは
    math.sqrt(64) = 8 (一辺の長さ) から角の2マスを引いて
    8 - 2 = 6
    6を何回埋められるか(4回まで)見ると
    24でちょうど4回。
    一辺（の残り部分）を埋める嬉しさは一個はめるとき2で角に到着した場合だけ3だから
    計算できる
    仮に足りないとすると、()
    
    一辺nの正方形部分の嬉しさは？線引いて、格子点を数えるみたいにすれば
    (l-1) ** 2とわかる
    """
    for t in range(int(input())):
        ...
        # 内側の正方形部分の辺の長さ
        l = math.sqrt(tiles)
        print(f"正方形部分の辺の長さ {l=}")

        # 正方形部分を加算
        ans += ((l-1) ** 2)
        print(f"正方形部分 = {l ** 2}, {ans=}")

        # 周り
        edge = N - 4**l
        print(f"周りに置く数 = {edge}, {ans=}")
        ...
        if 4 <= edge // 3:
            ans += 12
            edge -= 12
            print(f"4隅全部に置いた, {ans=}")
        else: ## 4つは置けない場合
            ans += (edge//3) * 4
            edge %= 3
            print(f"隅に置けるだけ置いた, {ans=}")
