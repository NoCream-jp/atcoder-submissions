...
def main():
    ...
    mostA, mostB = find_most(A), find_most(B)
    if mostA != mostB:
        print(calc(A, mostA) + calc(B, mostB))
    else:
        # 2番目の最頻値を使って，どちらがいいか比べる
        secondA = find_second(A)
        secondB = find_second(B)
        # print("second", secondA, secondB)
        ans = min(
            (calc(A, mostA) + calc(B, secondB)), (calc(A, secondA) + calc(B, mostB))
        )
        print(ans)
