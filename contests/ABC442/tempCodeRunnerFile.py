...
def main():
    ...
    """
    高橋くんバケモンやん
    角度を出して，角度で二分探索したいけどできる？
    傾きだと象限が異なったときに区別できないからダメ
    しかも二倍しないとややこしい

    ↑できそう，象限は最初からわかるから，どこにいるかから何度足せばいいかが分かる
    AとBのどっちがでかいかわからないのでA
    ...
    N, Q = i_map()
    monsters = []
    a = np.array([0, 0]) # 原点に固定しておく．aとbの角度を求める
    d = defaultdict(int) # にぶたん用
    for i in range(N):
        x, y = i_map()
        # 座標から角度作る
        b = np.array([x, y])
        vec = b - a
        arg = np.arctan2(vec[0], vec[1])
        if arg < 0:
            arg = (2 * np.pi) - arg # 0,1からみて左まわりは時計回りの角度に直す
        d[arg] += 1

    print(d)
    for k in d:
        monsters.append((k, d[k]))
    monsters.sort(key = lambda x: x[0])
    args, number = [], []
    for i in range(len(monsters)):
        args.append(monsters[i][0])
        number.append(monsters[i][1])
    ...
    for i in range(Q):
        ...
        aarg = d[a]
        barg = d[b]
        # こうすると固有の角度しかmonsterに入ってない
        aidx = bs.bisect_left(args, aarg)
        bidx = bs.bisect_left(args, barg) + 1
        print("aidx, bidx", aidx, bidx)
        print("aの数, bの数", numbers[aidx], numbers[bidx])
