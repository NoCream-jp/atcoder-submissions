...
def main():
    ...
    graph = [[] for _ in range(N)]  # 計算用

    uf = UnionFind(N)

    for i in range(N):
        graph[i].append((A[i] - 1, i + 1))  # (渡す先, 渡すコスト)
        uf.union(A[i] - 1, i)
    ...
    loops = {i: [] for i in roots}

    # 深さ優先探索でループを検出する。rootsの数だけ必要
    visited = [False for i in range(N)]
    for start in roots:
        stack = [(start, A[start], 1)]  # 重さ
        visited[start] = True
        while stack:
            now, nowcost, nowloopcount = stack.pop()
            for neighbor, neighborcost in graph[now]:
                newcost = nowcost + neighborcost
                newloopcount = nowloopcount + 1
                if visited[neighbor] == True:
                    loops[start] = (nowcost, nowloopcount)
                    break
                else:
                    visited[neighbor] = True
                    stack.append((neighbor, newcost, newloopcount))
    print(loops)
