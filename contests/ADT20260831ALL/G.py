...
def main():
    ...
    for i in range(int(input())):
        ...
        X = i_list()
        Y = i_list()
        dp = {w:[-INF for _ in range(N)] for w in ["s", "r"]}
        dp["s"][0] = 0 if S[0]=="S" else -X[0]
        dp["r"][0] = 0 if S[0]=="R" else -X[0]
        for i in range(1, N):
            # iがsになるのは s->sなのかr->sなのか
            # iがrになるのは s->rなのかr->rなのか
            if S[i] == "S":
                dp["s"][i] = max(dp["s"][i-1], dp["r"][i-1] + Y[i-1])
                dp["r"][i] = max(dp["s"][i-1] - X[i], dp["r"][i-1] - X[i])
            else:
                dp["s"][i] = max(dp["s"][i-1] - X[i], dp["r"][i-1] - X[i] + Y[i-1])
                dp["r"][i] = max(dp["s"][i-1], dp["r"][i-1])
    
        print(max(dp["s"][-1], dp["r"][-1]))
