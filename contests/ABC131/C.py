...
def main():
    ...
    lcm = getlcm(C, D)
    entire = B - A + 1
    divc = B // C - A // C + (1 if A % C == 0 else 0)
    divd = B // D - A // D + (1 if A % D == 0 else 0)
    divcd = B // lcm - A // lcm + (1 if A % lcm == 0 else 0)
    ans = entire - (divc + divd - divcd)
    # print(f"全体{entire}, {C}の倍数={divc}, {D}の倍数={divd}, {lcm}の倍数={divcd}")
    print(ans)
