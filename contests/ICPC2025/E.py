...
def main():
    ...
    while 1:
        ...
        if n == 0:
            break
        cost = [float('inf') for _ in range(n+1)] # 隣のコストのうち最小のものを保存
        parents = [-1 for _ in range(n+1)] # i番目の親はparents[i]
        ...
        print(f"cost:{cost}, parents:{parents}")
        heapq.heapify(cost) # スタート地点を選ぶための優先度付きキュー

        visited = [False for _ in range(n+1)]
        visited[1] = True
        while cost:
            start = heapq.heappop(cost)
            nxt = parents[start]
            print(f"{start} -> {nxt}")
            if visited[nxt]
            # while :
