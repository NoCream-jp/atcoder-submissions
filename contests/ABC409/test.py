...
def main():
    N, M, X = i_map()
    graph = [[] for _ in range(N)]  # [((行先, Trueなら自分から出る方向))]
    for i in range(M):
        u, v = i_map()
        u -= 1
        v -= 1
    ...
    visited[0] = True
    stack = [(0, 0, False)]  # (現在のノード，現在のコスト，現在の反転フラグ)
    ans = float("inf")
    while stack:
        now, cost, flag = stack.pop()
        for neighbor, direction in graph[now]:
            # 反転させてない場合，自分から出ている辺の先はstackに(flagそのまま)
            # 自分から出ていない辺の先は反転コスト含めてstackに(flag変える)
            if neighbor == N - 1:
                if (flag == True and direction == True) or (
                    flag == False and direction == False
                ):
                    ans = min(ans, cost + 1)
                else:
                    ans = min(ans, cost + 1 + X)
            if flag == False:
                if direction == True:  # flagはそのまま
                    stack.append((neighbor, cost + 1, flag))
                else:  # X足したうえでflagを反転
                    stack.append((neighbor, cost + 1 + X, not flag))
            else:
                if (
                    direction == True
                ):  # 反転させた状態で自分から出てる辺(が反転した辺)を見る
                    stack.append((neighbor, cost + 1 + X, not flag))
                else:  # 反転させた状態で自分に向かう辺が反転したものを見る
                    stack.append((neighbor, cost + 1, flag))
