...
def main():
    ...
    """
    そもそもペアで組むのが最善
    ペアがいない奴らのなかで最善の組み合わせを探せばよい。
    1 2 4 7 8
    で4をどうやって無視するか
    左右との距離の最小値を持たせて、それが最大の奴を無視する
    """
    if len(A) == 1:
        print(0)
        return
    ...
    for i in range(len(A)):
        if 0 <= i-1:
            weight[A[i]] = max(weight[A[i]], abs(A[i]-A[i-1]))
        if i+1 < len(A):
            weight[A[i]] = max(weight[A[i]], abs(A[i]-A[i+1]))
    mx = max(weight.values())
    ...
    for i in range(len(A)//2):
        lans += abs(A[2*i] - A[2*i + 1])
    A = A[::-1]
    rans = 0
    for i in range(len(A)//2):
        rans += abs(A[2*i] - A[2*i + 1])
    print(min(lans, rans))
