...
def main():
    ...
    マスと線じゃなくイチゴを主人公にして
    二分探索二回して自分がタテ何番目，ヨコ何番目のマスに入るか調べる．
    マスの情報を出して(i*(横幅) + jとかでいい)辞書の形で保存していく．
    辞書の中身は{(マスの情報: イチゴの個数)}
    にすれば，最後に辞書を走査するだけで解ける
    イチゴがおかれないマスがあると辞書にそもそも載らない可能性があるが，
    辞書の長さとケーキの個数を比べればそれもわかる
    """
    ...
    berries = []
    for _ in range(N):
        berries.append(tuple(i_map()))

    A = int(input())
    xlines = sorted(i_list())
    B = int(input())
    ylines = sorted(i_list())

    d = defaultdict(int)
    for p, q in berries:
        x = bisect.bisect_left(xlines, p)
        y = bisect.bisect_left(ylines, q)
        info = x * W + y
        # print(f"info: {info}")
        d[info] += 1
