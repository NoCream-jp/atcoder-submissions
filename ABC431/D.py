...
def main():
    ...
    smW = sum(W)
    dp = [[-1 for _ in range(smW * 2 + 1)] for __ in range(2)]
    dp[0][smW] = 0

    for i in range(N):  # 一個前は%の違うほう
        for j in range(smW * 2 + 1):
            if dp[i % 2][j] == -1:
                continue
            if j - W[i] < 0:
                pass
                ...
                dp[(i + 1) % 2][j - W[i]] = max(
                    dp[i % 2][j] + H[i], dp[(i + 1) % 2][j - W[i]]
                )
            if smW * 2 < j + W[i]:
                pass
            else:
                dp[(i + 1) % 2][j + W[i]] = max(
                    dp[i % 2][j] + B[i], dp[(i + 1) % 2][j + W[i]]
                )
