...
def main():
    ...
    if 0 < tempsum:
        while lstack:
            i, value = lstack.pop() # valueは長さ
            L, R = ans[i]
            if 0 < tempsum - value:
                tempsum -= value
                ans[i] = ((L, L))
            else: # tempsumが0になってくれる回
                ans[i] = ((R - tempsum, R - tempsum))
                break
        if not lstack and tempsum:
            print("No")
            exit()
    else:
        while rstack:
            ...
            if tempsum + value < 0:
                ...
                ans[i] = ((L + tempsum, L + tempsum))
                break
        if not rstack and tempsum:
            print("No")
            exit()
