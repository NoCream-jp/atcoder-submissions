...
def main():
    """
    素数を列挙するには空間O(N)いるのでできなさそう

    aは5乗根まで(300とかまで)しか大きくならない
    から、Nからaを列挙する。
    それぞれのaについてN//aがbccの上限。
    bはbccの3乗根まで(8000とかまで)しか大きくならない
    から、N//(a**2)からbを列挙する。
    それぞれのbについてN//bがccの上限。
    """
    ...
    N = int(input())
    count = 0
    for a in get_primes(2, int(N ** (1 / 5)) + 1):
        # print("a", a)
        bcc = N // a**2
        for b in get_primes(a + 1, int((bcc) ** (1 / 3)) + 1):
            # print("b", b)
            cc = bcc // b
            for c in get_primes(b + 1, int((cc) ** (1 / 2)) + 1):
                if a * a * b * c * c <= N:
                    # print(a*a*b*c*c)
                    count += 1
                else:
