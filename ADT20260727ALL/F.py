...
def main():
    ...
    # 左上(a, b), 右下(c, d)として
    a1, b1, c1, d1, a2, b2, c2, d2 = INF, INF, 0, 0, INF, INF, 0, 0
    for i in range(N):
        for j in range(N):
            if grid1[i][j] == "#":
                a1, b1 = min(a1, i), min(b1, j)
                c1, d1 = max(c1, i), max(d1, j)
            if grid2[i][j] == "#":
                a2, b2 = min(a2, i), min(b2, j)
                c2, d2 = max(c2, i), max(d2, j)
    ...
    newgrid1 = [["." for _ in range(C1)] for _ in range(R1)]
    for i in range(R1):
        for j in range(C1):
            newgrid1[i][j] = grid1[a1 + i][b1 + j]

    newgrid2 = [["." for _ in range(C2)] for _ in range(R2)]
    for i in range(R2):
        for j in range(C2):
            newgrid2[i][j] = grid2[a2 + i][b2 + j]

    ans = "No"
    for _ in range(4):
        newgrid1 = rotate(newgrid1)
        if newgrid1 == newgrid2:
            ans = "Yes"
    print(ans)
