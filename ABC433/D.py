...
def main():
    ...
    剰余を辞書にいったん入れて、対応するやつがあればその個数が答え
    を辞書内全部についてやればいい
    のはf()が足し算の時の話で、今回みたいに足される奴が何倍かされてる場合わからない

    たかだか2項でつくったf()は元の数から10桁離れた値しか使わないので、
    二次元にして10個ずつずらしたものをもたせよう

    2, 42なら
    2と20と200と2000と...を格納して
    42, 420, 4200...を格納して
    42 と 200とかでそろってれば数える

    42についてみるとき、必ずペアは「100倍された後のやつらの辞書」
    にしか探しに行かなくていい
    """

    dl = [{} for _ in range(11)]  # i(0 ~ 10) 番目は 10 ** i 倍したやつのMでの剰余
    for a in A:
        for i in range(11):
            num = a * (10**i)
            if num % M not in dl[i]:
                dl[i][num % M] = 1
            else:
                dl[i][num % M] += 1
    ans = 0
