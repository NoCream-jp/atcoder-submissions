...
def main():
    ...
    """
    必ず葉から根に集める必要がある。部分木について見ると、その全体のどこかに電子が
    散らばっているとした場合に必ず+-の間の辺の数コストがかかる。つまり、必ずどちらかを
    動かす必要がある。それなら根に集めたほうがいい
    散らばっているか終わっているかは数えればわかる
    ...
    w_graph = [defaultdict(int) for _ in range(N)]
    ...
    goal = sum([(e[i] if 0 < e[i] else 0) for i in range(len(e))])
    ans = 0
    e_count = 0 # 出会ったぶん絶対値を足す。Nになったら終わり
    while q and e_count != goal:
        # 葉を見る
        now = q.popleft()
        for nxt in graph[now]:
            # 葉をむしる
            graph[nxt].remove(now)
            # 次に葉になってしまったら見たい葉に入れる
            if len(graph[nxt]) == 1:
                q.append(nxt)
            else:
                pass
            # 電子があれば動かして答えに加算する
            ans += abs(e[now]) * w_graph[now][nxt]
            if e[now] * e[nxt] < 0:
                # +5 と -7 なら 2 を加算
                e_count += min(abs(e[now]), abs(e[nxt]))
            # nxtに集めて-2を記録
            e[nxt] = e[now] + e[nxt]
    print(ans)
