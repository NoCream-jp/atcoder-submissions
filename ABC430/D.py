...
def main():
    ...
    """
    群れの中に人が割り込んだ時、総和は変わる
    にぶたんで見つけた左右とくらべて計算するだけ？
    """

    ans = 0
    sl = SortedList([[0, 0]])  # 位置とmin距離
    for x in X:
        ...
        if len(sl) == 1:
            ...
            continue
        if idx == 0:
            rightnum, rightvalue = sl[idx]
            sl.discard[rightnum, rightvalue]
            sl.add([rightnum, abs(rightnum - x)])
            sl.add([x, abs(rightnum - x)])
            if abs(rightnum - x) < rightvalue:
                ans += abs(rightnum - x) * 2 - rightvalue
            else:
                ans += abs(rightnum - x)
            # print(f"左端 right={rightnum}")
        elif idx == len(sl):
            leftnum, leftvalue = sl[idx - 1]
            # print(f"右端 left={leftnum}")
            sl.discard([leftnum, leftvalue])
            sl.add([leftnum, abs(leftnum - x)])
            sl.add([x, abs(leftnum - x)])
            if abs(leftnum - x) < leftvalue:
                ans += abs(leftnum - x) * 2 - leftvalue
            else:
                ans += abs(leftnum - x)
        else:  # このばあいansからの引き算もいる
            leftnum, leftvalue = sl[idx - 1]
            rightnum, rightvalue = sl[idx]
            # print(f"left={leftnum}, right={rightnum}")
            # print(leftnum, leftvalue, rightnum, rightvalue)
            sl.discard([leftnum, leftvalue])
            sl.discard([rightnum, rightvalue])
            ans -= rightvalue + leftvalue
            if abs(leftnum - x) < abs(rightnum - x):  # 左のほうとの距離を採用(左、自分)
                sl.add([x, (abs(leftnum - x))])
                ans += abs(leftnum - x) * 2 + abs(rightnum - x)
            else:
                sl.add([x, abs(rightnum - x)])
                ans += abs(rightnum - x) * 2 + abs(leftnum - x)
            sl.add([leftnum, abs(leftnum - x)])
            sl.add([rightnum, abs(rightnum - x)])
        # print(f"ans={ans}\n")
        print(ans)
